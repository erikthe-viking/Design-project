...
def preprocess_card(contour, image):
    ...
    # Grab the right corner of the wraped card image and do a 4x zoom
    QcornerR = qCard.warp[0:CORNER_WIDTH+15, 200-60:200]
    dst = imutils.rotate_bound(QcornerR, -90)
    QcornerRT = cv2.resize(dst, (CORNER_WIDTH, CORNER_HEIGHT))
    QcornerR_zoom = cv2.resize(QcornerRT, (0,0), fx=4, fy=4)
    #cv2.imshow("QcornerR",QcornerR)
    #cv2.imshow("Rotation",dst)
    #cv2.imshow("RZoom", QcornerR_zoom)
    ...
    white_level = QcornerR_zoom[15, int((CORNER_WIDTH*4)/2)]
    thresh_level = white_level - CARD_THRESH
    if (thresh_level <= 0):
        thresh_level = 1
    retval, query_threshR = cv2.threshold(QcornerR_zoom, thresh_level, 255, cv2.THRESH_BINARY_INV)
    ...
    # Crop the top right corner transform's top
    QrankR = query_threshR[20:185, 0:128]
    #cv2.imshow("QrankR", QrankR)

    # Find rank contour and bounding rectangle, isolate and find largest contour
    dummy, Qrank_cnts, hier = cv2.findContours(Qrank, cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    Qrank_cnts = sorted(Qrank_cnts, key=cv2.contourArea,reverse=True)
    
    dummy, QrankR_cnts, hierR = cv2.findContours(QrankR, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    QrankR_cnts = sorted(QrankR_cnts, key=cv2.contourArea, reverse=True)
...
def match_card(qCard, train_ranks, train_suits):
    ...
    if (len(qCard.rank_img) != 0 or len(qCard.suit_img)):   
        ...
        if(len(qCard.suit_img) != 0):
            ...
            # Modified to check alt rank
            for Trank in train_ranks:
                
                    diff_img = cv2.absdiff(qCard.suit_img, Trank.img)
                    suit_diff = int(np.sum(diff_img)/255)
                    #cv2.imshow("diff_img", diff_img)
                    if suit_diff < best_rank_match_diff:
                        best_rank_diff_img = diff_img
...
def draw_results(image, qCard):
    ...
    cv2.putText(image,(rank_name),(x-30,y-10),font,1,(0,0,0),3,cv2.LINE_AA)
    cv2.putText(image,(rank_name),(x-30,y-10),font,1,(50,200,200),2,cv2.LINE_AA)
